...
//        if(dataFile.exists()) dataFile.delete();
    ...
    public void testLoginSuccess() {
        // 注册一个用户以供测试
        this.userAdmin.register("testUser", "testPass123");
    ...
    public void testLoginFail() {
        ...
        // 注册一个用户以供测试
        userAdmin.register("testUser", "testPass123");
    ...
    public void testResigterCheck() {
        ...
        assertEquals("注册检查应该通过",
                ...
                userAdmin.registerCheck("validUser", "validPass123"));

        // 测试账号长度小于3
        assertEquals("账号太短，注册检查应该失败",
                UserAdministrator.ResigterError.ACCOUNT_ILLEGAL,
                userAdmin.registerCheck("a","1234567"));
        //测试账号长度等于3
        assertEquals("注册检查应该通过",
                UserAdministrator.ResigterError.No_Error,
                userAdmin.registerCheck("abc","1234567"));

        //测试账号长度大于3小于13
        assertEquals("注册检查应该通过",
                UserAdministrator.ResigterError.No_Error,
                userAdmin.registerCheck("abcd","1234567"));
        //测试账号长度等于13
        assertEquals("注册检查应该通过",
                UserAdministrator.ResigterError.No_Error,
                userAdmin.registerCheck("abcdefghijklm","1234567"));
        // 测试账号包含特殊字符空格
        assertEquals("账号有特殊字符，注册应该失败",
                UserAdministrator.ResigterError.ACCOUNT_ILLEGAL,
                userAdmin.registerCheck("abc defg","123456"));


        // 测试密码长度小于7
        assertEquals("密码太短，注册检查应该失败",
                UserAdministrator.ResigterError.PASSWORD_ILLEGAL,
                userAdmin.registerCheck("abcdefg","1234"));
        // 测试密码长度等于7
        assertEquals("账户密码符合要求，注册检查应该成功",
                UserAdministrator.ResigterError.No_Error,
                userAdmin.registerCheck("abcdefg","1234567"));
        // 测试密码长度大于7小于19
        assertEquals("账户密码符合要求，注册检查应该成功",
                UserAdministrator.ResigterError.No_Error,
                userAdmin.registerCheck("abcdefg","1122334455667788990"));
        // 测试密码长度大于19
        assertEquals("账户密码过长，注册检查应该失败",
                UserAdministrator.ResigterError.PASSWORD_ILLEGAL,
                userAdmin.registerCheck("abcdefg","11223344556677889900"));


        // 测试密码包含非法字符
        assertEquals("密码含有非法字符，注册检查应该失败",
                UserAdministrator.ResigterError.PASSWORD_ILLEGAL,
                userAdmin.registerCheck("abcdefg","123 456"));

        // 测试账户和密码为空
        assertEquals("输入为空，注册检查应该失败",
                UserAdministrator.ResigterError.ACCOUNT_ILLEGAL,
                userAdmin.registerCheck("", ""));
    }
    ...
    public void testResigter() {
        // 测试注册成功
        User user = userAdmin.register("newUser", "newPass123");
        assertNotNull("注册失败，应该成功注册", user);

        // 测试重复注册
        User user2 = userAdmin.register("ExistedUser", "newPass123");
        assertEquals("账户已存在，注册检查应该失败",
                UserAdministrator.ResigterError.ACCOUNT_EXIST,
                userAdmin.registerCheck("ExistedUser", "anotherPass123"));
    }
    @Test
